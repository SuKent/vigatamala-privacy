...
App 啟動或回前景時，以 HTTPS 傳送安裝 UUID、App bundle ID 與平台 ios，建立或找回裝置帳號；後續請求帶 Keychain 保存的 session token。發現 Plus 交易時傳送交易編號、原始交易編號、產品 ID、正式／沙盒環境、已有的 appAccountToken 及 Apple 簽章交易。後端向 Apple 覆核，必要時為 App 外兌換的交易補上 token，再查詢訂閱狀態。**不傳送瀏覽歷史、網址、搜尋、書籤、播放內容或位置。** 連線服務仍會處理 IP 等必要連線資訊。這些資料只用於帳號關聯與訂閱服務，不作廣告追蹤。刪除 App 或清除瀏覽資料不會刪除後端紀錄；需刪除或查詢帳號資料，請聯絡第 9 節信箱。**刪除會移除裝置綁定、授權與訂閱狀態。但有兩樣不會跟著走:一是裝置本身的紀錄(含安裝識別碼)——它不隨帳號刪除而消失,要等到無活動 90 天後才自動移除,涉及濫用防範的情形除外;二是交易紀錄與 Apple 簽章的原始通知本身,它們會保留、只抹去與你的關聯**——那是處理退款與購買爭議時的憑證。帳號與交易資料保留於提供服務及處理購買爭議所需期間;裝置活躍度紀錄預設保留 180 天。舊版未接 CISIP 的建置不發出這些請求。
